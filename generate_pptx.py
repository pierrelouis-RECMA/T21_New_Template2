...
def clean_slide(slide):
    ...
    for shape in list(slide.shapes):
        # On garde les logos ou fonds si nécessaire, mais on retire les tableaux et textes de données
        if shape.has_table or shape.has_text_frame:
            # Optionnel : on peut filtrer pour ne pas supprimer les titres
...
def load_stats(file_path):
    ...
    df['Agency'] = df['Agency'].astype(str).str.strip().upper()
    df['NewBiz'] = df['NewBiz'].astype(str).str.strip().upper()

    agencies = []
    for ag in df.Agency.unique():
        if ag == 'NAN' or ag == '': continue
        sub = df[df.Agency == ag]
        w = sub[sub.NewBiz=='WIN']['Integrated Spends'].sum()
        d = sub[sub.NewBiz=='DEPARTURE']['Integrated Spends'].sum()
        agencies.append({
            'agency': ag, 'nbb': w+d, 'wins': w, 'dep': d,
...
def generate_report(input_excel_path):
    ...
    # 1. Remplacement texte global (Titre, Pays)
    replace_text_globally(prs, "Hong Kong", market_name)
    replace_text_globally(prs, "HONG KONG", market_name)

    # 2. Nettoyage des slides de données pour éviter les superpositions
    # Slide 2 (Key Findings), 3 (TOP), 4 (Overview), 5 (Retentions), 6 (Details)
    for idx in [1, 2, 3, 4, 5]: 
        if idx < len(prs.slides):
            clean_slide(prs.slides[idx])

    # 3. Application du design moderne (Slide 3 & 4)
